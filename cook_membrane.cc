...
//    const Point<dim> bottom_left = (dim == 3 ? Point<dim>(0.0, 0.0, -0.5) : Point<dim>(0.0, 0.0));
  //  const Point<dim> top_right = (dim == 3 ? Point<dim>(48.0, 44.0, 0.5) : Point<dim>(48.0, 44.0));
        const Point<dim> top_right =Point<dim>(0.0, 0.0, -22); 
        const Point<dim> bottom_left =Point<dim>(88,44, 22);
        ...
        if (cell->face(face)->at_boundary() == true)
          {
            if (std::abs(cell->face(face)->center()[0] - 0.0) < tol_boundary)
              cell->face(face)->set_boundary_id(1); // -X faces
            else if (std::abs(cell->face(face)->center()[0] - 88.0) < tol_boundary)
              cell->face(face)->set_boundary_id(11); // +X faces
            else if (dim == 3 && std::abs(std::abs(cell->face(face)->center()[2]) - 22) < tol_boundary)
              cell->face(face)->set_boundary_id(2); // +Z and -Z faces
          }
...
/*    const Point<dim> soln_pt = (dim == 3 ? 
  ...
  */
const Point<dim> soln_pt = Point<dim>(88*parameters.scale, 44*parameters.scale, 22*parameters.scale);
  ...
  protected:
                                           ...
                                           PerTaskData_ASM &data)
    {
      ...
      for (unsigned int face = 0; face < GeometryInfo<dim>::faces_per_cell;
		...
		const double magnitude  = (1.0/(22.0*parameters.scale*1.0*parameters.scale))*time_ramp;
